...
def call(params, logger):
    ...
    ⑦文書種別ごと　のX二乗検　実施
    """
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001001", ["s3_chi_square_doc"]))
    ...
    if len(target_list) != 0:
        ...
        for filepath in filepathlist:
            filname = os.path.basename(filepath).replace('_形態素解析.csv', '')
            ...
            for target, p_value in zip(target_list, p_value_list):
                ...
                # X二乗結果　ALL
                outpath =outDir + target + '/karute/' + filname + '_X二乗.csv'
    ...
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001002", ["s3_chi_square_doc"]))
