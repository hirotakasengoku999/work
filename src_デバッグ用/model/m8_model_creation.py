...
from src.model.f7_randomforest.r1_randomforest_learning import call as r1  # 1
from src.model.f7_randomforest.r2_randomforest_learning_special_case import call as r2  # 2

def m8_main(config, logger, model_path):
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001001", ["m8_model_creation"]))
    # 処理は昇順に実行する
    for key, params in sorted(config.items()):
        try:
            if key == "1":
                r1(params, logger, model_path)
            elif key == "2":
                r2(params, logger, model_path)
            elif key == "_comment":
                pass
            else:
                # 固有処理の呼び出し
            ...
            raise
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001002", ["m8_model_creation"]))
