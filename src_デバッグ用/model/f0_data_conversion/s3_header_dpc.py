...
def call(params, logger):
    ...
    患者ごとに紐づけ番号、結合キーを付与
    '''
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001001", ["s3_header_dpc"]))
    ...
    for target_month2 in s_target_month:
        out_path = out_dir + target_month2 + '/dpc/'

        # 出力フォルダ作成
        os.makedirs(out_path, exist_ok=True)

        for kubun in ['国保', '社保']:
            file_path = in_dir + target_month2 + '/dpc/RECEIPTD' + kubun
            ...
            if len(file_list) == 0:
                ...
                # 先にREの処理を実施
                # 空じゃない場合、 REがなかったら　エラー　AIR003008　中間ファイル名
                ...
                if len_dfre - len_head == 2:
                    header_list = header_list + key
                elif len_dfre - len_head > 2:
                    header_list = header_list + [''] * (len_dfre - len_head - 2) + key
                else:
                ...
                # 出力
                out_str = out_path + '/RECEIPTD' + kubun + '_RE.csv'
                try:
                ...
                for file in file_list:
                    ...
                    df_out = pd.merge(df, df_re_karute, on=key)
                    ...
                    try:
                        df_out.to_csv(out_str, index=False, encoding='cp932')
                    except:
                        import traceback, sys
                        l = lambda x: x[0](x[1]);l(logger.format_log_message("AIR003015", [out_str+ ":" + traceback.format_exception(*sys.exc_info())[-1]]))
                        raise

    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001002", ["s3_header_dpc"]))
