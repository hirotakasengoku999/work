	...
	for index, v in enumerate(rece_obj):
		...
		for row in rows:
			...
			if row_list[0] == 'RE':
				...
				date_of_execution = row_list[1] if len(row_list) > 1 else ''
				medical_category = row_list[2] if len(row_list) > 2 else ''
				receipt_code = converting_to_name('m_medical_act', '診療行為', row_list[5]) if len(row_list) > 5 else ''
				count = row_list[9] if len(row_list) > 9 else ''
				reaf_cd_dict = {
