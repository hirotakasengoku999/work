...
def write_result(input_path, seikyunengetsu):
	...
	for file in files_file:
		...
		item_code = file[:-4]
		d[4] = code_name_dict[item_code]
		d.insert(4,'',item_code)
		list_file.append(d)
	...
	# 同じ請求年月のレコードを削除する
	sql = ("DELETE FROM aireceiptapp_kenchikekka WHERE SEIKYUNENGETSU=%s")
	...
	df_all.to_sql('aireceiptapp_kenchikekka', con=con, if_exists='append', index=None)
...
if __name__ == '__main__':
	import folder
	input_path = folder.result()
	seikyunengetsu = '2021-03'
	write_result(input_path, seikyunengetsu)