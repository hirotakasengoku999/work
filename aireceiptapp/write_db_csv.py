...
def write_result(input_path, seikyunengetsu, result_insert_rules: dict):
    ...
    word_replacement_codes: list = result_insert_rules.get("insert_word_replacement_codes", [])

    # csvファイルの結合
    list_file = []
    for file in files_file:
        item_code = file[:-4]
        if not item_code in word_replacement_codes:
            d = pd.read_csv(input_path + file, engine='python', skiprows=1, encoding='cp932', header=None,
                            usecols=range(0, 30))
    ...
    df_all.columns = ['USER_CHECK', 'SEIKYUNENGETSU', 'PATIENT_ID', 'PATIENT_NAMEJ', 'PATIENT_NAMEK',
                      'MEDICAL_CARE_DATE', 'IN_OUT', 'DEPT', 'WARD', 'CODE', 'CODENAME', 'CODE2', 'CODE2NAME', 'POINTS',
                      'PREDICT_PROBA', 'ZISSEKI', 'KENCHI01', 'KENCHI02', 'KENCHI03', 'KENCHI04', 'KENCHI05',
                      'KENCHI06', 'KENCHI07', 'KENCHI08', 'KENCHI09', 'KENCHI10', 'KENCHI11', 'KENCHI12', 'KENCHI13',
                      'KENCHI14', 'KENCHI15', 'KENCHI16', 'KENCHI17', 'KENCHI18', 'KENCHI19', 'KENCHI20', 'KENCHI21',
                      'KENCHI22', 'KENCHI23', 'KENCHI24', 'KENCHI25', 'KENCHI26', 'KENCHI27', 'KENCHI28', 'KENCHI29',
                      'KENCHI30', 'KENCHI31', 'KENCHI32', 'KENCHI33', 'KENCHI34', 'KENCHI35', 'KENCHI36', 'KENCHI37',
                      'KENCHI38', 'KENCHI39', 'KENCHI40', 'KENCHI41', 'KENCHI42', 'KENCHI43', 'KENCHI44', 'KENCHI45',
                      'KENCHI46', 'KENCHI47', 'KENCHI48', 'KENCHI49', 'KENCHI50', 'OUTPUTDATE']
...
if __name__ == '__main__':
    ...
    import ent_updated_result, folder
    ...
    # 検知結果の挿入ルールを読み込む
    result_insert_rules_path = Path(folder.base()) / 'config' / 'result_rules.json'
    with open(result_insert_rules_path, 'r') as f:
        result_insert_rules = json.load(f)

    write_result(input_path, seikyunengetsu, result_insert_rules)

    # 特殊処理（複数の根拠を置き換え）
    import insert_results_word_replacement

    word_replacement_codes: list = result_insert_rules.get("insert_word_replacement_codes", [])
    for target_code in word_replacement_codes:
        try:
            insert_results_word_replacement.write_result(input_path, seikyunengetsu, target_code,
                                                         is_target_word_only=True)
        except:
