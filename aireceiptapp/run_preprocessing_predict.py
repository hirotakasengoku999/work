...
try:
    ...
    from . import folder, get_model_list, update_chart
except ImportError:
    # 単体実行時のインポート
    import folder, get_model_list, update_chart
import time
import MySQLdb
import datetime
...
def update_proc_status(update_status, criteria_date=datetime.date.today()):
  ...
  sql = ("UPDATE aireceiptapp_hanyo SET CONTROLINT1 = %s WHERE CODE1 = 'proc' AND CODE2 = 'predict' AND STARTDATE <= %s AND ENDDATE >= %s")
  params = (update_status, criteria_date, criteria_date,)
  cursor.execute(sql, params)
  sql = ("UPDATE aireceiptapp_hanyo SET CONTROLTEXT1 = %s WHERE CODE1 = 'proc' AND CODE2 = 'predict' AND STARTDATE <= %s AND ENDDATE >= %s")
...
def update_cannot_start_log(process_name, criteria_date=datetime.date.today()):
  ...
  cursor = connection.cursor(MySQLdb.cursors.DictCursor)
  sql = ("SELECT NAME FROM aireceiptapp_hanyo WHERE CODE1 = 'menu' AND CODE2 = '02' AND STARTDATE <= %s AND ENDDATE >= %s")
  params = (criteria_date, criteria_date)
  cursor.execute(sql, params)
  rows = cursor.fetchall()
  proc_name = '検知'
...
if __name__ == '__main__':
  ...
  if proc_status == 0:
    ...
    if get_CONTROLINT2() == 0:
      request_sql = "UPDATE aireceiptapp_hanyo SET CONTROLTEXT2 = 'run_karte_pp' WHERE CODE1 = 'proc' AND CODE2 = 'predict'"
      update_aireceiptdb(request_sql)
      execution_program(os.path.join(folder.base(), "aireceiptapp", "run_karte_pp.py"), "")
    if get_CONTROLINT2() == 0:
      request_sql = "UPDATE aireceiptapp_hanyo SET CONTROLTEXT2 = 'run_predict' WHERE CODE1 = 'proc' AND CODE2 = 'predict'"
      update_aireceiptdb(request_sql)
      model_folder = os.path.join(folder.model(),'model')
      modelfile_list = get_model_list.model_list(model_folder)
      codes = get_model_list.list_str_conversion(modelfile_list)
      execution_program(os.path.join(folder.base(), "aireceiptapp", "run_predict.py"), codes)
    update_CONTROLINT2()
    print('ステータスを更新します')
    update_proc_status(0)
    request_sql = "UPDATE aireceiptapp_hanyo SET CONTROLTEXT2 = 'comp' WHERE CODE1 = 'proc' AND CODE2 = 'predict'"
    update_aireceiptdb(request_sql)
    logger.info('検知結果を集計します')
