...
try:
    ...
    from . import get_kenchi, get_hanyo
except ImportError:
    # 単体実行時のインポート
...
def write_add_calc(table_name, seikyunengetsu, chart_table='add_calc'):
    print(f'★★★★chart_table = {chart_table}')
...
def get_max_outputdate(seikyunengetsu):
    request_sql = "SELECT MAX(OUTPUTDATE) AS max_outputdate FROM aireceiptapp_kenchikekka WHERE SEIKYUNENGETSU = '"
