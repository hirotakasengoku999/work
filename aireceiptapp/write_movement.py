...
def write_movement(input_path):
	...
	for i, v in all_df.iterrows():
		...
		move_datetime_h = move_datetime.replace('/', '-')
		move_category = v['MOVE_CATEGORY']
		ward = v['WARD']
		dept = v['DEPT']
		row_df = pd.read_sql(sql=f"select * from aireceiptapp_movement_info where patient_id = '{patient_id}' and REPLACE(move_datetime, '/', '-') like '{move_datetime_h}%%' and move_category = '{move_category}'", con=con)
		if len(row_df) > 0:
			with con.connect() as connection:
				connection.execute(f"update aireceiptapp_movement_info set ward = '{ward}', dept = '{dept}' where patient_id = '{patient_id}' and REPLACE(move_datetime, '/', '-') like '{move_datetime_h}%%' and move_category = '{move_category}'")
		else:
