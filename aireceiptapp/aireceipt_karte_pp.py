...
def aireceipt_karte_pp(input_path, output_path):
    ...
    for path_1 in file_listc:
        ...
        # 共通
        df = RenameColum(df, {'患者番号': karute, '記事日時': data_column_name, '手術日付': data_column_name, '実施日': data_column_name, '手術実施日': data_column_name, '採取日': data_column_name, '期間開始日時': data_column_name, '科名': dept_column_name, '科名称': dept_column_name})
        ...
        elif basename.startswith('DVT予防法'):
            basename = 'DVT予防法'
