...
from .forms import HanyoForm, ResultForm, KenchiForm, SystemLogSelectForm, UserListSelectForm, KarteFileForm, KarteFileFormColumn, ResultColumnForm, ChartSelect, AddItemAllForm
from . import folder, proc_flg_update, check_validation, get_hanyo, paginator, move_dir, receipt_pre_chk, karte_pre_chk, get_kenchi, get_kenchikekka, karte_conversion, check_user, read_kartefile, get_request_amount, get_chart, get_inspectiondb, read_log, update_chart_app, update_results, remove_old_data
import sys
import os
import traceback
import subprocess
...
def createmodel(request):
  ...
  rece_chk_files = get_hanyo.get_hanyo(f"SELECT NAME, CONTROLTEXT1, CONTROLTEXT2 FROM aireceiptapp_hanyo WHERE CODE1 = 'receipt_file_chk' AND STARTDATE <= '{t}' AND ENDDATE >= '{t}'")
...
def run_predict(request):
  ...
  # 個別処理 TYK 入院履歴.csvと移動情報.csvをマージし、レイアウト仕様書通りの移動情報.csvを生成する s
  try:
    logger.info('tyk_merge_movementを開始します')
    from . import tyk_merge_movement
    tyk_merge_movement.main(user_karte_pre_folder)
    logger.info('tyk_merge_movement完了')
  except:
    logger.warning(sys.exc_info())
    logger.warning(traceback.format_exc())
  # 個別処理 TYK 入院履歴.csvと移動情報.csvをマージし、レイアウト仕様書通りの移動情報.csvを生成する e
  try:
  ...
  if len(in_files) > 0:
    ...
    # 移動情報ファイル名を取得
    in_out_file_t = get_hanyo.get_hanyo(
      "select file_name from aireceiptapp_karte_file where file_code like 'mv%%' limit 1")
    in_out_file = in_out_file_t[0]['file_name']
    in_out_file = in_out_file.split('.')[0]

    karte_pre_file_list = read_kartefile.get_karte_pre_file_list()
    for in_file in in_files:
      shutil.copy(in_file, Path_admin_karte_backup_folder/in_file.name)

      # 移動情報のファイルでない場合、karte_preフォルダーに移動する
      if not in_file.name.startswith(in_out_file):
...
def test_paginator(request):
  ...
  if display_predict_proba_dict:
    any_evidence_inpatient_codes = set()
    try:
      result_rules_path = Path(base) / 'config' / 'result_rules.json'
      with open(result_rules_path, 'r', encoding='utf-8') as f:
        result_rules = json.load(f)
      any_evidence_inpatient_codes = set(result_rules.get('get_result_any_evidence_codes', []))
    except:
      pass
    add_proba_sql += " AND ("
    for k, v in display_predict_proba_dict.items():
      if k == '111000110':
        ...
        where_sql = f" (CODE = '{k}' AND PREDICT_PROBA >= {str(v)} AND IN_OUT = '入院' AND WARD IN ('ＩＣＵ・ＣＣＵ')) OR"
      elif k == '113006910':
        where_sql = f" (CODE = '{k}' AND PREDICT_PROBA >= '{str(v)}' AND IN_OUT = '入院' AND KENCHI16 = '0.0' AND(KENCHI01 <> '' OR KENCHI03 <> '' OR KENCHI05 <> '' OR KENCHI07 <> '' OR KENCHI09 <> '')) OR"
      elif k in any_evidence_inpatient_codes:
