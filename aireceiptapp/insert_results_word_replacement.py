...
def write_result(input_path, seikyunengetsu, target_code, is_target_word_only=False):
    ...
    try:
        ...
        pass
    try:
        file = base / f'aireceipt/data/receipt_code/add_word_{target_code}.csv'
        wordmaster_df = pd.read_csv(file, engine='python', encoding='cp932', dtype='object')
        wordmaster_df['コード'] = wordmaster_df['コード'] + "_" + wordmaster_df['名称']
        wordmaster_list = wordmaster_df['コード'].to_list()
    except:
        pass
    ...
    if len(list_file) > 0:
        ...
        preserved_values_dict = {}
        for source_col, target_col in preserve_column_dict.items():
            if source_col in list(df_all.columns):
                preserved_values_dict[target_col] = df_all[source_col].reset_index(drop=True)
        usecols = []
        for col in base_cols + karutemaster_list + wordmaster_list + symaster_list + simaster_list:
        ...
        for col in list(df_all.columns)[13:]:
            ...
            if col in karutemaster_list + wordmaster_list + symaster_list + simaster_list:
                value_unique = list(df_all[col].unique())
        ...
        for target_col, preserved_values in preserved_values_dict.items():
            outdf[target_col] = preserved_values.reindex(range(len(outdf))).where(preserved_values.reindex(range(len(outdf))).notna(), "")
        ...
        df_no_calculation_results['ZISSEKI'] = '0'
        ...
        df_calculation_exists_results['ZISSEKI'] = '1'
        ...
        df_no_calculation_results = df_no_calculation_results.fillna('')
        df_calculation_exists_results = df_calculation_exists_results.fillna('')
