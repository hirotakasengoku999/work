...
def write_movement(input_path):
	...
	in_out_col_t = get_hanyo.get_hanyo("SELECT BEFORE_COLUMN_NAME, DATA_TYPE FROM aireceiptapp_karte_file_column WHERE FILE_CODE LIKE 'mv%'")
	in_out_col_d = {item['DATA_TYPE']: item['BEFORE_COLUMN_NAME'] for item in in_out_col_t}
	usecols = list(in_out_col_d.values())
	...
	for file in input_path.glob('**/*.csv'):
		if in_out_file in file.name:
			...
			in_df = df[usecols]
			in_df = in_df.rename(columns={in_out_col_d['患者ID']:"PATIENT_ID", in_out_col_d['日付']:"MOVE_DATETIME", in_out_col_d['移動通知']:'MOVE_CATEGORY', in_out_col_d['病棟']: "WARD"})
	...
	for i, v in all_df.iterrows():
		...
		if len(row_df) > 0:
			with con.connect() as connection:
				connection.execute(f"update aireceiptapp_movement_info set ward = '{ward}' where patient_id = '{patient_id}' and move_datetime like '{move_datetime}%%' and move_category = '{move_category}'")
		else:
