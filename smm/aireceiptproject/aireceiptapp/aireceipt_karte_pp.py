...
def aireceipt_karte_pp(input_path, output_path):
    ...
    for path_1 in file_listc:
        ...
        # 共通
        df = RenameColum(df, {'患者コード': karute, 'カルテ日付': data_column_name, '手術日': data_column_name})
        if '手術歴' in basename:
            basename = '手術歴'
            df = Grouping(df, 'カルテ内容', ['術式１'])
            df = RenameColum(df, {'開始時間': '手術開始時間'})
            df = df[[karute, data_column_name, '手術開始時間', 'カルテ内容']]
        ...
        elif '医師記録' in basename:
            basename = '医師記録'
            df = df[[karute, data_column_name, '診療科', 'カルテ内容']]

        elif '看護記録' in basename:
            basename = '看護記録'
            df = df[[karute, data_column_name, '診療科', 'カルテ内容']]

        df[data_column_name] = pd.to_datetime(df[data_column_name]).dt.strftime("%Y/%m/%d")
        df[karute] = df[karute].str.zfill(7)
        # df.to_csv(os.path.join(output_path, f"{basename}.csv"), index=False, encoding='cp932') # 確認用
