...
from src.predict.f5_individual_correspondence.f_calculations_count.s_calculations_count import \
    call as calculations_count  # 6


def p2_main(config, logger, heap_path, in_targets):
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001001", ["p2_receipt_processing_option"]))
    # 処理は昇順に実行する
    for key, params in sorted(config.items()):
        try:
            if key == "1":
                ...
                calculations_count(params, logger, heap_path, in_targets)
            elif key == "_comment":
                pass
            else:
                # 固有処理の呼び出し
            ...
            raise
    l = lambda x:x[0](x[1]); l(logger.format_log_message("AIR001002", ["p2_receipt_processing_option"]))
