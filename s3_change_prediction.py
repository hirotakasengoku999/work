...
def call(params, logger, in_targets):
    ...
    option_pattern_5 = option_pattern[4]
    ...
    # ○必要な算定項目がなければ強制0%
    def Check5(targetNo, required_codes, option):
        indf = Input(targetNo)
    ...
    for target, required, option in zip(target_list_5, drop_list_5, option_pattern_5):
        # in_targetsにあれば実施
        if target in in_targets:
            Check5(target, required, option)
